...
const { getAllRoles, getAllDepartments, getAllEmployees, addManager } = require('./db/db_access');
...
const addEmployees = () => {
    db.getAllRoles().then(([rows]) => {
        ...
        db.addManager().then(([rows]) => {
            let manager = rows
            const managerChoices = manager.map(({ id, manager_id }) => {
            });
            inquirer.prompt([
                {
                    type: 'input',
                ...
                .then(res => {
                    ...
                    db.addEmployee(emp)
                        ...
                        .then(() => init())
                    console.log(`
                    Added employee to the company database
                    `);
                })
        });
    };
    ...
    const employeeRole = () => {
        inquirer.prompt([
            ...
            .then
        console.log(`
        Updated employee's role in company database
        `);
        //runs start function again 
        init();
    };
            ...
            inquirer.prompt([
                ...
                .then(res => {
                    ...
                    db.addEmpRole(role)
                        ...
                        .then(() => init())
                    console.log(`
                    Added role to the company database
                    `);
                })
        });
    };
        ...
        inquirer.prompt([
            ...
            .then(res => {
                ...
                db.addDepartment(dept)
                    ...
                    .then(() => init())
                console.log(`
                Added department to company database
                `);
            })
    };
